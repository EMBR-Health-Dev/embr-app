          ...
          if (existing) {
            for (const [key, value] of Object.entries(update)) {
              if (
                ...
                existing.noticedAreas = (value as { set: string[] }).set;
              } else {
...
describe("GET /onboarding", () => {
  ...
  it("only ever returns the caller's own profile, never another user's", async () => {
    ...
    await agentA.patch("/onboarding").send({
      jobToBeDone: "KEEP_RECORD",
    });
...
describe("PATCH /onboarding", () => {
  it("requires authentication", async () => {
    ...
    const res = await request(app).patch("/onboarding").send({
      currentStep: "WELCOME",
    });
  ...
  it("rejects an invalid enum value", async () => {
    ...
    const res = await agent.patch("/onboarding").send({
      jobToBeDone: "SOMETHING_MADE_UP",
    });
  ...
  it("rejects an invalid array element in noticedAreas", async () => {
    ...
    const res = await agent.patch("/onboarding").send({
      noticedAreas: ["SLEEP", "NOT_A_REAL_AREA"],
    });
  ...
  it("supports genuine partial updates without clobbering previously-set fields", async () => {
    ...
    const first = await agent.patch("/onboarding").send({
      jobToBeDone: "UNDERSTAND_PATTERNS",
    });

    expect(first.status).toBe(200);
    expect(first.body.data.jobToBeDone).toBe("UNDERSTAND_PATTERNS");

    const second = await agent.patch("/onboarding").send({
      noticedAreas: ["SLEEP", "ENERGY"],
    });
  ...
  it("persists currentStep for resume", async () => {
    ...
    await agent.patch("/onboarding").send({
      currentStep: "JOB_TO_BE_DONE",
    });
  ...
  it("cannot modify another user's profile", async () => {
    ...
    await agentB.patch("/onboarding").send({
      jobToBeDone: "NOT_SURE",
    });
...
describe("onboardingCompletedAt on /auth/me", () => {
  ...
  it("stays set on subsequent logins after a skip too", async () => {
    ...
    const reLogin = await agent.post("/auth/login").send({
      email: "persistentskip@embr.health",
      password: VALID_PASSWORD,
    });
